// <copyright file="AssemblyLoader.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Introspection
{
    ...
    public class AssemblyLoader
    {
        ...
        public static AssemblyDefinition LoadCecilAssembly(string fileName, bool immediateLoad = false,
            bool? readSymbols = null)
        {
            using (var t = new Tracer(Level.L5, MyType, "LoadCecilAssembly"))
            {
                string pdbPath = Path.ChangeExtension(fileName, "pdb");
                bool tryReadSymbols = readSymbols ?? File.Exists(pdbPath);
                ...
                try
                {
                    ReadingMode readingMode = immediateLoad ? ReadingMode.Immediate : ReadingMode.Deferred;
                    var assemblyResolver = new DefaultAssemblyResolver();
                    assemblyResolver.AddSearchDirectory(fileInfo.Directory.FullName);
                    var readerParameters = new ReaderParameters
                        { ReadSymbols = tryReadSymbols, ReadingMode = readingMode, AssemblyResolver = assemblyResolver };
