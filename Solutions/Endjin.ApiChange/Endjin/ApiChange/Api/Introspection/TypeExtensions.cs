// <copyright file="TypeExtensions.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Introspection
{
    ...
    public static class TypeExtensions
    {
        ...
        public static FieldDefinition GetFieldByNameAndType(this List<FieldDefinition> list, string fieldName,
            string fieldType)
        {
            FieldDefinition lret = null;

            foreach (FieldDefinition field in list)
            {
        ...
        public static EventDefinition GetEventByNameAndType(this List<EventDefinition> list, string evName, string type)
        {
            ...
            foreach (EventDefinition ev in list)
            {
                if (ev.Name == evName)
                {
                    if (type == null)
        ...
        private static void PrintGenericArgumentCollection(StringBuilder sb, Collection<TypeReference> coll)
        {
        ...
        /// <param name="sb"></param>
        /// <param name="coll"></param>
        private static void PrintGenericParameterCollection(StringBuilder sb, Collection<GenericParameter> coll)
        {
            if (coll == null || coll.Count == 0)
            {
                return;
        ...
        private static void PrintParameters(Collection<ParameterDefinition> parameters, StringBuilder sb,
            MethodPrintOption options)
        {
        ...
        {
            var sb = new StringBuilder();
            bool bPrintAlias = (MethodPrintOption.ShortNames & MethodPrintOption.ShortNames) ==
                               MethodPrintOption.ShortNames;

            if ((MethodPrintOption.Visiblity & options) == MethodPrintOption.Visiblity)
            {
                AddVisibility(sb, method.IsPublic, method.IsPrivate, method.IsFamily, method.IsAssembly,
            ...
            if ((MethodPrintOption.ReturnType & options) == MethodPrintOption.ReturnType)
            {
                ...
                if (bPrintAlias)
                {
                    var generic = method.ReturnType as GenericInstanceType;
                    if (generic != null)
                    {
        ...
        private static bool Contains(this Collection<TypeReference> interfaces, TypeReference itfTypeRef)
        {
        ...
        public static bool IsEqual(this Collection<ParameterDefinition> x, Collection<ParameterDefinition> y,
            bool bCompareGenericParameters)
        {
            if (x == null && y == null)
            {
                return true;
        ...
        {
            Func<TypeDefinition, FieldDefinition, bool> filter = FilterFunctions.GetFieldFilter(mode);
        ...
        {
            Func<TypeDefinition, MethodDefinition, bool> filter = FilterFunctions.GetMethodFilter(mode);
        ...
        {
            Func<TypeDefinition, EventDefinition, bool> filter = FilterFunctions.GetEventFilter(mode);
