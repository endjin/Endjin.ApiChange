// <copyright file="MethodQuery.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Query
{
    ...
    public class MethodQuery : BaseQuery
    {
            ...
            : base(methodQuery)
        {
            ...
            if (!m.Success)
            {
                throw new ArgumentException(string.Format(
                    "Invalid method query: \"{0}\". The method query must be of the form <modifier> <return type> <function name>(<arguments>) e.g. public void F(*) match all public methods with name F with 0 or more arguments, or public * *(*) match any public method.",
                    methodQuery));
            }
        ...
        {
            string filter = m.Groups["retType"].Value.Replace(" ", string.Empty);
        ...
        internal bool MatchReturnType(MethodDefinition method)
        {
            ...
            return this.ReturnTypeFilter.IsMatch(method.ReturnType.FullName);
        }
        ...
        {
            List<MethodDefinition> matches = this.GetMethods(type);
            if (matches.Count > 1)
            {
                throw new InvalidOperationException(string.Format(
                    "Got more than one matching method: {0}",
                    matches.Count));
            }
