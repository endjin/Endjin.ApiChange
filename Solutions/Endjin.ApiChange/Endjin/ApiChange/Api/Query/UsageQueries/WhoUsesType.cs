// <copyright file="WhoUsesType.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Query.UsageQueries
{
    ...
    public class WhoUsesType : UsageVisitor
    {
        ...
        public override void VisitType(TypeDefinition type)
        {
            ...
            foreach (InterfaceImplementation typeRef in type.Interfaces)
            {
                if (this.IsMatching(this.myArgSearchTypeNames, this.mySearchArgTypes, typeRef.InterfaceType, out matchingType))
                {
        ...
        public override void VisitMethod(MethodDefinition method)
        {
            ...
            if (this.IsMatching(this.myArgSearchTypeNames, this.mySearchArgTypes, method.ReturnType, out matchingType))
            {
        ...
        public override void VisitMethodBody(MethodBody body)
        {
            ...
            foreach (Instruction instr in body.Instructions)
            {
                ...
                if (instr.OpCode.Code == Code.Newobj || instr.OpCode.Code == Code.Call ||
                    instr.OpCode.Code == Code.Calli || instr.OpCode.Code == Code.Callvirt)
                {
                    ...
                    if (this.IsMatching(this.myArgSearchTypeNames, this.mySearchArgTypes, typeRef, out matchingType))
                    {
                        ...
                        else
                        {
                            context = new MatchContext(
                                ...
                                string.Format(
                                    "{0} on type {1}",
                                    calledMethod == null ? string.Empty : calledMethod.Print(MyMethodDisplayMode),
                                    typeRef.FullName));
                        }
