// <copyright file="UsageQueryAggregator.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Query.UsageQueries
{
    ...
    public class UsageQueryAggregator : IDisposable
    {
        ...
        /// </param>
        public UsageQueryAggregator(string symbolServer)
            : this(true)
        {
        ...
        public UsageQueryAggregator(TypeQuery query)
            : this(true)
        {
            this.myTypeQuery = query;
        }

        public List<QueryResult<MethodDefinition>> MethodMatches { get; }

        public List<QueryResult<TypeDefinition>> TypeMatches { get; }

        public HashSet<string> AssemblyMatches { get; }

        public List<QueryResult<FieldDefinition>> FieldMatches { get; }
        ...
        {
            string fileName = Path.GetFileName(dllName).Replace(".dll", string.Empty).Replace(".exe", string.Empty);
        ...
        public void Analyze(AssemblyDefinition assembly)
        {
            using (var t = new Tracer(MyType, "Analyze"))
            {
                ...
                foreach (ModuleDefinition mod in assembly.Modules)
                {
                    ...
                    {
                        t.Info(
                            "Current assembly does not reference any assemblies that are checked. Skip other checks.");
                        continue;
                    }

                    foreach (TypeDefinition type in this.myTypeQuery.GetTypes(assembly))
                    {
                        this.myVisitors.ForEach(vis => vis.VisitType(type));

                        foreach (FieldDefinition field in FieldQuery.AllFieldsIncludingCompilerGenerated
        ...
        public void AddMatch(Instruction ins, MethodDefinition method, bool bSearchForward, MatchContext context)
        {
            using (var t = new Tracer(MyType, "AddMatch_Instruction"))
            {
                ...
                QueryResult<MethodDefinition> queryResult =
                    this.GetResultWithFileLineIfEnabled(ins, method, bSearchForward, context);
        ...
        private bool hasLoadedPdbInfos(MethodBody body)
        {
            ...
            {
                SequencePoint seqPoint = body.Method.DebugInformation.GetSequencePoint(inst);
                if (seqPoint != null)
                {
                    return true;
                }
        ...
        /// <returns></returns>
        private QueryResult<MethodDefinition> GetResultWithFileLineIfEnabled(Instruction ins, MethodDefinition method,
            bool bSearchForward, MatchContext context)
        {
            QueryResult<MethodDefinition> ret = null;
            if (this.myPdbReader != null)
            {
                KeyValuePair<string, int> fileLine = this.myPdbReader.GetFileLine(ins, method, bSearchForward);
                ret = new QueryResult<MethodDefinition>(method, fileLine.Key, fileLine.Value, context);
            }
            else
            {
                ret = new QueryResult<MethodDefinition>(method, string.Empty, 0, context);
            }
        ...
        internal void AddMatch(MethodDefinition method, MatchContext context)
        {
            ...
            else
            {
                if (this.myPdbReader != null)
                {
                    KeyValuePair<string, int> fileLine = this.myPdbReader.GetFileLine(method.DeclaringType);
                    this.MethodMatches.Add(
                        new QueryResult<MethodDefinition>(method, fileLine.Key, fileLine.Value, context));
                }
                else
                {
                    this.MethodMatches.Add(new QueryResult<MethodDefinition>(method, string.Empty, 0, context));
                }
            }
        }
    }
