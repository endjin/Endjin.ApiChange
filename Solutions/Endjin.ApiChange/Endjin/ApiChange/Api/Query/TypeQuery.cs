// <copyright file="TypeQuery.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Query
{
    ...
    public class TypeQuery
    {
        ...
        public TypeQuery(TypeQueryMode mode, string namespaceFilter, string typeNameFilter)
        {
            ...
            if (!this.IsEnabled(TypeQueryMode.Class) && !this.IsEnabled(TypeQueryMode.Interface) &&
                !this.IsEnabled(TypeQueryMode.ValueType) && !this.IsEnabled(TypeQueryMode.Enum))
            {
        ...
        private void CheckSearchMode(TypeQueryMode mode)
        {
            ...
            if (!this.IsEnabled(mode, TypeQueryMode.Interface) && !this.IsEnabled(mode, TypeQueryMode.Class) &&
                !this.IsEnabled(mode, TypeQueryMode.ValueType))
            {
                throw new ArgumentException(
                    "You must search for a class, interface or value type in your module. See QueryMode enumeration for values");
            }
        }
        ...
        {
            List<TypeDefinition> types = new TypeQuery().GetTypes(assembly);
        ...
        private bool TypeMatchesFilter(TypeDefinition typeDef)
        {
            ...
            if (this.CheckVisbility(typeDef))
            {
                ...
                {
                    if (typeDef.IsSpecialName || typeDef.Name == "<Module>" ||
                ...
                TypeDefinition decl = typeDef.DeclaringType;
                while (decl != null && decl.Namespace == string.Empty)
                {
                ...
                if (this.IsEnabled(TypeQueryMode.Class) && typeDef.IsClass &&
                    !(typeDef.IsInterface || typeDef.IsValueType))
                {
                    lret = true;
                }
