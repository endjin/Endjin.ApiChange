// <copyright file="DirectorySearcherAsync.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Infrastructure
{
    ...
    public class DirectorySearcherAsync : IEnumerable<string>
    {
        ...
        public DirectorySearcherAsync(string searchPath, string searchPattern, SearchOption searchOption)
        {
            ...
            Tracer.Info(Level.L1, MyType, "DirectorySearcherAsync", "Search in path {0} for {1}", this.mySearchPath,
                searchPattern);
        ...
        public bool HasMatchingFiles
        {
            ...
            {
                this.BeginSearch();
                int idx = WaitHandle.WaitAny(new WaitHandle[] { this.myHasFileEvent, this.myHasFinishedEvent });
                return idx == 0;
            }
        }
        ...
        private void FindFileThread()
        {
            using (var t = new Tracer(Level.L3, MyType, "FindFileThread"))
            {
                try
                {
                    try
                    {
                        ...
                        while (fileenum.MoveNext())
                        {
                            lock (this)
                            {
                                ...
                                t.Info("Found file {0}, Listening Queue Count: {1}", fileenum.Current,
                                    this.myListeningQueues.Count);

                                foreach (BlockingQueue<string> listeningQueue in this.myListeningQueues)
                                {
                    ...
                    {
                        t.Error(ex, "Got error while searching in {0} for pattern {1}.", this.mySearchPath,
        ...
        public BlockingQueue<string> GetResultQueue()
        {
            using (var t = new Tracer(Level.L3, MyType, "GetResultQueue"))
            {
                ...
                lock (this)
                {
                    foreach (string foundFile in this.myFiles)
                    {
                        queue.Enqueue(foundFile);
                    }

                    t.Info("Queue did get {0} files we found so far. Current State: {1}, Path: {2}", this.myFiles.Count,
                        this.mySearchState, this.mySearchPath);
