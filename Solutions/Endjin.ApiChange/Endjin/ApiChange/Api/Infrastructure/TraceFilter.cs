// <copyright file="TraceFilter.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Infrastructure
{
    ...
    internal class TraceFilter
    {
        ...
        public TraceFilter(string typeFilter, MessageTypes msgTypeFilter, Level levelFilter, TraceFilter next)
        {
            ...
            string[] parts = typeFilter.Trim().ToLower().Split(new[] { '.' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public virtual bool IsMatch(TypeHashes type, MessageTypes msgTypeFilter, Level level)
        {
            ...
            if (lret)
            {
                bool areSameSize = this.myFilterHashes.Length == type.myTypeHashes.Length;

                for (int i = 0; i < this.myFilterHashes.Length; i++)
                {
