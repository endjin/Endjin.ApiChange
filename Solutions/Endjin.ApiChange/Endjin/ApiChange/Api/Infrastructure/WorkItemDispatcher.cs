// <copyright file="WorkItemDispatcher.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Infrastructure
{
        ...
        where T : class
    {
        ...
        public WorkItemDispatcher(int width, Action<T> processor, BlockingQueue<T> work, WorkItemOptions options)
            : this(new WorkItemDispatcherData<T>
            {
                Width = width,
                Processor = processor,
                InputData = work,
        ...
        /// <param name="options">Exception handling options.</param>
        public WorkItemDispatcher(int width, Action<T> processor, BlockingQueueAggregator<T> workList,
        ...
        public WorkItemDispatcher(int width, Action<T> processor, string name, BlockingQueue<T> work)
            : this(
            width,
            processor, name, work, WorkItemOptions.Default)
        {
        }
        ...
        public WorkItemDispatcher(WorkItemDispatcherData<T> data)
        {
            ...
            for (int i = 0; i < this.myData.Width; i++)
            {
                IAsyncResult res = processWork.BeginInvoke(this.Completed, null);
        ...
        public void Dispose()
        {
            using (var t = new Tracer(MyType, "Dispose"))
            {
                if (!this.myIsDisposed)
                {
                    ...
                    foreach (WaitHandle wait in this.myExitEvents)
                    {
                        wait.Close();
                    }
            ...
            using (var tr = new Tracer(MyType, "ProcessWork"))
            {
                try
                {
                    while (true)
                    {
                        ...
                        T work = this.GetNextWorkItem();
                        if (work == null || this.myExceptions.Count > 0 &&
                                this.IsEnabled(WorkItemOptions.ExitOnFirstEror))
                        {
                        ...
                        {
                            tr.Error(Level.L2, ex, "Got exception in worker thread {0} while processing work item {1}",
                                this.Name, work);
                ...
                {
                    tr.Error(Level.L1, ex, "Worker thread {0} was interrupted by exception of worker thread",
                        this.Name);
                    lock (this.myExceptions)
                    {
                        this.myExceptions.Add(ex);
