// <copyright file="InternalError.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Infrastructure
{
    ...
    {
        private static readonly string Help = "Tracing can be enabled via the environment variable " +
                                              TracerConfig.TraceEnvVarName + Environment.NewLine +
                                              ...
                                              Environment.NewLine +
                                              "                       Default             Write traces to configured trace listeners with the source name " +
                                              TracerConfig.TraceEnvVarName + " read from App.config" +
                                              Environment.NewLine +
                                              ...
                                              Environment.NewLine + "                     Example: Level1+InOut+Info" +
                                              Environment.NewLine +
                                              "                     When no severity and or trace level is specified all levels/severities are enabled." +
                                              Environment.NewLine +
                                              " Enable full tracing (all severities and all levels) to debugoutput for all types except the ones which reside in the ApiChange.Infrastructure namespace" +
                                              Environment.NewLine + "    " + TracerConfig.TraceEnvVarName +
                                              "=debugoutput; ApiChange.* all;!ApiChange.Infrastructure.* all" +
                                              Environment.NewLine +
                                              " Enable file traces with Level1 for all types except the ones beneath the ApiChange.Infrastructure namespace" +
                                              Environment.NewLine + "    " + TracerConfig.TraceEnvVarName +
                                              "=file; * Level1;!ApiChange.Infrastructure.* all" + Environment.NewLine +
                                              " Trace all exceptions in the method where it is first encountered" +
                                              Environment.NewLine + "    " + TracerConfig.TraceEnvVarName +
                                              "=file c:\\temp\\exceptions.txt; * Exception";
