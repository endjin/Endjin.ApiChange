// <copyright file="Tracer.cs" company="Endjin">
...
namespace Endjin.ApiChange.Api.Infrastructure
{
    ...
    public struct Tracer : IDisposable
    {
        ...
        [ThreadStatic] private static Exception myLastPrintedException;
        ...
        public Tracer(TypeHashes type, string method)
            : this(Level.L1, type, method)
        {
        }
        ...
        public void Dispose()
        {
            if (TracerConfig.Instance.IsEnabled(this.myType, MessageTypes.Exception, this.myLevel))
            {
                ...
                if (currentException != null && ReferenceEquals(myLastPrintedException, currentException) == false)
                {
                    ...
                    TraceMsg(MsgTypeException, this.TypeMethodName, DateTime.Now, "Exception thrown: {0}",
                        currentException);
                }
            }
            ...
            if (TracerConfig.Instance.IsEnabled(this.myType, MessageTypes.InOut, this.myLevel))
            {
                ...
                TraceMsg(MsgTypeOut, this.TypeMethodName, now, "Duration {0}",
                    this.FormatDuration(now.Ticks - this.myEnterTime.Ticks));
            }
        }
        ...
        public void Error(Level level, Exception ex, string fmt, params object[] args)
        {
            ...
            if (TracerConfig.Instance.IsEnabled(this.myType, MessageTypes.Error, level))
            {
                ...
                TraceMsg(MsgTypeError, this.TypeMethodName, DateTime.Now, "{0}{1}{2}", traceMsg, Environment.NewLine,
                    ex);
            }
        }
        ...
        /// <param name="fmt">Message describing what the exception is about.</param>
        /// <param name="args">Optional format arguments for the description message string.</param>
        public static void Error(Level level, TypeHashes type, string method, Exception ex, string fmt,
            params object[] args)
        {
            if (ex == null)
            {
            ...
            {
                string traceMsg = FormatStringSafe(fmt, args);
                TraceMsg(MsgTypeError, GenerateTypeMethodName(type, method), DateTime.Now, "{0}{1}{2}", traceMsg,
        ...
        private static void TraceMsg(string msgTypeString, string typeMethodName, DateTime time, string fmt,
            params object[] args)
        {
            string traceMsg = FormatStringSafe(fmt, args);
            if (TraceEvent != null)
            ...
            traceMsg = string.Join(" ", FormatTime(time), TracerConfig.Instance.PidAndTid, msgTypeString,
                typeMethodName, traceMsg, Environment.NewLine);
        ...
        private static string FormatTime(DateTime time)
        {
            ...
            int q = n1 * 50 + ((50 * (n2 >> 16)) >> 16) - (int)(Math.BigMul(1244382467, n1) >> 32) - 1;
            int r = (int)(ticks - Math.BigMul(q, 86400000));
            if (r > 86400000)
            {
                r -= 86400000;
            }
